...
class Main extends React.Component {
    ...
    _collectionMemberList = async (pageIndex, params = this.state.queryObj) => {
        try {
            ...
            const {overdueStageId, nameOrphone, isAllocation, operator, createStartTime, createEndTime, companyId} = params;
            const newParams = {
                overdueStageId,
                nameOrphone,
                isAllocation,
            ...
            const result = await Http.collectionMemberList(newParams);
            if (!result) {
                return;
            }
            const {errcode, errmsg, data} = result;
                        ...
                        <Col {...ColConfig}>
                            <Form.Item {...FormItemLayout} label='操作员' style={{marginBottom: '0px'}}>
                                {getFieldDecorator('operator', {
                                    rules: [{required: false, message: '请输入操作员姓名/手机'}],
                                })(
...
const CollectionMemberSetForm = Form.create()(Main);
